...
all: tecnicofs tecnicofs-client $(PROG_BINS)

# Special case to bring tecnicofs binaries to the root level directory
tecnicofs: build/bin/tecnicofs
	@echo $@: Moving binary
	@cp '$<' '$@'
tecnicofs-client: build/bin/tecnicofs-client
	@echo $@: Moving binary
	@cp '$<' '$@'
